...
"""
Defines the tests for both async TCP/RTU hosts.
"""


async def _read_coils_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
...
async def _write_coils_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
    new_coil_val = 0
...
async def _read_hregs_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
...
async def _write_hregs_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
    new_hreg_val = 44
...
async def _write_hregs_beyond_limits_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
...
async def _read_ists_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
...
async def _read_iregs_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
...
async def _reset_registers_test(host, slave_addr, register_definitions, sleep_fn, **kwargs):
...
async def run_host_tests(host, slave_addr, register_definitions, exit_on_timeout=False):
